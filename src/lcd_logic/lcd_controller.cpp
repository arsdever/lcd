...
namespace lcd
{
	std::unordered_map<lcd_controller::command_types_enum, std::chrono::nanoseconds> lcd_controller::s_execution_time_map =
	{
	...
	lcd_controller::lcd_controller()
		...
		, m_hscroll{ 0 }
		, m_cursor_show{ false }
		, m_cursor_move_direction{ cursor_direction_enum::decrement }
		, m_insert{ false }
		, m_blink{ false }
		, m_busy{ false }
		, m_display_on{ false }
		, m_lines{ false }
		, m_font{ false }
		, m_scroll_direction{ false }
		, m_ddram{ 0 }
	{
		m_port.m_pins[static_cast<int>(pinout::en)].on_edge_down(std::bind(&lcd_controller::port_updated_callback, this, pinout::en));
	}

	void lcd_controller::register_for_updates(on_update_delegate callback)
	{
		m_on_update_cb = callback;
	}
