...
namespace lcd
{
	...
	public:
		...
		requires is_pin<pin_t>
			static inline bool get(pin_t const& pin)
		{
			return pin.get_voltage() > 3.0f;
		}
	};
	...
	//	template <typename pin_t = default_digital_pin>
	//	requires is_pin<pin_t>
	//		inline static constexpr void set_state(std::shared_ptr<pin_t> pin, bool state) { return pin->set_voltage(op_voltage); }
	//};
